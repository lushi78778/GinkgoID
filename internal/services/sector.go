...
func validateSectorIdentifier(cfg config.Config, uri string, redirects []string) error {
    ...
    if timeout <= 0 { timeout = 0 } // 使用 http 默认超时（无超时）或保持为 0
    ...
    for _, u := range arr { have[u] = true }
    for _, r := range redirects { if !have[r] { return fmt.Errorf("missing redirect_uri %s", r) } }
    return nil
}
