...
func (s *TokenService) VerifyJWT(tokenStr string) (jwt.MapClaims, error) {
	...
	_, err := jwt.ParseWithClaims(tokenStr, claims, func(t *jwt.Token) (interface{}, error) {
		if kid, ok := t.Header["kid"].(string); ok && kid != "" {
			if rec, err := s.keys.FindByKid(context.Background(), kid); err == nil {
				return publicKeyFromRecord(rec), nil
			}
		}
		if rec, err := s.keys.ActiveKey(context.Background()); err == nil {
			return publicKeyFromRecord(rec), nil
		}
	...
	// 回退：依次尝试所有密钥（公钥）
	list, lerr := s.keys.AllKeys(context.Background())
	if lerr != nil {
		return nil, err
	}
	for _, k := range list {
