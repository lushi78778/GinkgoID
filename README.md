...
- 份 DB：mysqldump -u root -p ginkgoid > ginkgoid_$(date +%F).sql

---

如需我把 README 自动写入仓库（覆盖当前 README.md），或把某一部分扩展为独立文档（例如“Key 轮换 / DB 迁移 脚本”），请指示要我执行的具体项（写入 README / 添加迁移脚本 / 生成 docs/swagger.json 等）。
...
如果你不打算集成外部 KMS，本项目提供一个可选的本地私钥对称加密方案：

- 在 `config.yaml` 中设置 `crypto.key_encryption_key` 为一段高熵字符串（建议 32 字节）。
- 启用后，启动时新生成的签名私钥会在写入 MySQL 前使用 AES-256-GCM 加密；运行时签名操作会在内存中解密并用于签发 JWT。公钥仍以 PEM 明文存储并通过 JWKS 暴露。
- 注意事项：加密密钥本身需要妥善保管；将其放在受限的环境变量或运行时配置中。该方案相比直接明文存库提升了攻击难度，但不具备 KMS/HSM 的防护与审计能力。
...
该功能为简易本地方案，若需企业级密钥管理，仍建议集成云 KMS 或 Vault。

### OpenAPI / Swagger 维护说明

项目使用 Swagger 注释配合 `swag` 工具生成 OpenAPI 规范并由 Stoplight Elements 渲染。更新接口注释后请运行：

```bash
go run github.com/swaggo/swag/cmd/swag init --generalInfo cmd/server/main.go --output docs
```

生成后的 `docs/swagger.json` 会被 `web/stoplight.html` 加载用于展示。请在提交 PR 前确保文档与代码同步。
