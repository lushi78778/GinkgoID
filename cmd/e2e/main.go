...
func main() {
    ...
    headers := http.Header{}
    if initialAccess != "" { headers.Set("Authorization", "Bearer "+initialAccess) }
    var reg clientInfo
    ...
    // Fragment params
    frag := ""
    if i := strings.Index(loc2, "#"); i >= 0 { frag = loc2[i+1:] }
...
func cloneValues(v url.Values) url.Values {
    ...
    for k, vs := range v { c[k] = append([]string(nil), vs...) }
    return c
}
...
func doJSON(client *http.Client, method, urlStr string, body any, headers http.Header, want int, out any) error {
    ...
    if verbose { log.Printf("%s %s -> %d\n响应体: %s", method, urlStr, resp.StatusCode, prettyJSON(b)) }
    if out != nil { if err := json.Unmarshal(b, out); err != nil { return err } }
    return nil
}
...
func expectStatusJSON(client *http.Client, u *url.URL, want int, headers http.Header) error {
    ...
    if headers != nil { req.Header = headers.Clone() }
    req.Header.Set("Accept", "application/json")
    resp, err := client.Do(req)
    if err != nil { return err }
    defer resp.Body.Close()
...
func mustGetRedirect(client *http.Client, u *url.URL) (int, string) {
    req, _ := http.NewRequest("GET", u.String(), nil)
    resp, err := client.Do(req)
    if err != nil { log.Fatalf("GET %s: %v", u, err) }
    defer resp.Body.Close()
    ...
    loc := resp.Header.Get("Location")
    if verbose { log.Printf("GET %s -> %d Location=%s", u, resp.StatusCode, loc) }
...
func extractParam(loc, key string) string {
    ...
    raw := ""
    if i := strings.Index(loc, "#"); i >= 0 { raw = loc[i+1:] } else if i := strings.Index(loc, "?"); i >= 0 { raw = loc[i+1:] }
...
func randString(n int) string {
    ...
    if _, err := crand.Read(seed[:]); err == nil {
        rnd := int64(0)
        for i := 0; i < 8; i++ { rnd = (rnd << 8) | int64(seed[i]) }
        r := mrand.New(mrand.NewSource(rnd))
        for i := range b { b[i] = alphabet[r.Intn(len(alphabet))] }
    } else {
        for i := range b { b[i] = alphabet[mrand.Intn(len(alphabet))] }
    }
    return string(b)
}
...
func mustToken(client *http.Client, urlStr, id, secret string, form url.Values) tokenSet {
    ...
    req.Header.Set("Accept", "application/json")
    resp, err := client.Do(req)
    if err != nil { log.Fatalf("POST %s: %v", urlStr, err) }
    defer resp.Body.Close()
    b, _ := io.ReadAll(resp.Body)
    ...
    var ts tokenSet
    if err := json.Unmarshal(b, &ts); err != nil { log.Fatalf("token json: %v", err) }
    if verbose {
...
func prettyJSON(b []byte) string {
    var js any
    if err := json.Unmarshal(b, &js); err != nil { return safeTrunc(string(b), 1200) }
